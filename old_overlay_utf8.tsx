...
export function SalesCoachOverlay() {
  ...
  const salespersonStream = useSTTStream(undefined, (result, speakerCode) => {
    ...
    if (isDiarized) {
      ...
      if (identifiedSpeaker === 'prospect') {
        addLog(`≡ƒÄñ PROSPECT: "${result.text.substring(0, 30)}..."`)
      ...
      if (identifiedSpeaker === 'prospect') {
        addLog("≡ƒöä HEARTBEAT: Turn complete, refreshing WebSocket...")
  ...
  const prospectStream = useSTTStream((transcript) => {
    ...
    if (isDuplicate) {
      console.log(`[Prospect onSpeechEnd] ΓÅ¡∩╕Å SKIPPING duplicate transcript (within ${DEDUP_WINDOW_MS}ms window)`)
      return
    }
    ...
    if (handleTranscriptRef.current) {
      console.log(`[Prospect onSpeechEnd] Γ£à Calling handleTranscript...`)
      handleTranscriptRef.current(transcript.text, 'prospect')
      console.log(`[Prospect onSpeechEnd] Γ£à handleTranscript completed`)
    } else {
      console.error(`[Prospect onSpeechEnd] Γ¥î handleTranscriptRef.current is NULL!`)
    }
  })
  ...
  const resetCoachingState = useCallback(() => {
    ...
    addLog(`≡ƒöä Turn ${trace.turnId} Complete - Trace Reset (Session: ${prospectStream.sessionId || 'Ready'})`)
  ...
  const runCoaching = useCallback(async (turns: TranscriptTurn[], speaker: 'salesperson' | 'prospect', force = false) => {
    ...
    if (!isProspect) {
      addLog(`ΓÅ¡∩╕Å Skip: Speaker is ${speaker}, not prospect`)
      return
    }

    // Prevent duplicate coaching requests
    if (isCoachingInProgressRef.current) {
      addLog(`ΓÅ│ Skip: Coaching already in progress (blocking)`)
      return
    }
    ...
    if (!turnCheck.shouldGenerate) {
      addLog(`ΓÅ¡∩╕Å Skip: TurnManager rejected (${turnCheck.reason})`)
      return
    }
    ...
    const safetyResetTimer = setTimeout(() => {
      if (isCoachingInProgressRef.current) {
        console.warn('[runCoaching] ΓÜá∩╕Å Safety reset triggered after 10s - resetting isCoachingInProgress')
        addLog(`ΓÜá∩╕Å Coaching timeout [Turn ${nextTurnId}] - Resetting...`)
        isCoachingInProgressRef.current = false
    ...
    addLog(`≡ƒÜÇ COACHING TRIGGERED [Turn ${nextTurnId}]: ${speaker} - "${lastTurn.text.substring(0, 30)}..."`)
    ...
    try {
      ...
      addLog(`Γ£à AI Replied (${elapsed}ms)`)
      ...
      if (!safeSuggestion || safeSuggestion.length < 5) {
        ...
        addLog(`ΓÜá∩╕Å Using fallback suggestion`)
      }
      ...
      console.log(`[Prospect AI Response] ≡ƒöä Turn complete. Starting NEW prospect stream...`)
      if (isDiarized) {
        // Force Hardware Reset after AI card to ensure clean state
        addLog("ΓÜí SYSTEM: HARDWARE RESET (Immediate Post-Response)...")
        setTimeout(() => {
          console.warn("[IN-ROOM WATCHDOG] ≡ƒöä Refreshing audio pulse to prevent timeout...")
          salespersonStream.startAutomatic()
        }, 100)
      } else {
      ...
      console.log(`[Prospect AI Error] ≡ƒöä Error turn complete. Starting NEW prospect stream...`)
      prospectStream.startAutomatic()
    }
  ...
  const handleTranscript = useCallback((text: string, speaker: 'salesperson' | 'prospect' = 'salesperson') => {
    ...
    addLog(`≡ƒÄ» SPEECH END detected for ${speaker}: "${text.substring(0, 30)}..."`)

    const turn: TranscriptTurn = { speaker, text, timestamp: new Date().toISOString() }
    transcriptTurnsRef.current.push(turn)

    if (speaker === 'prospect') {
      addLog(`ΓÜí TRIGGERING AI COACHING for prospect speech`)
      runCoaching([...transcriptTurnsRef.current], speaker)
    } else {
      addLog(`Γä╣∩╕Å Salesperson speech detected - storing transcript only`)
    }
  }, [runCoaching, addLog])
  ...
  useEffect(() => {
    ...
    if (!activeStream.isSpeaking && activePartial && !isCoachingInProgressRef.current) {
      ...
      const timer = setTimeout(() => {
        addLog(`ΓÅ▒∩╕Å SILENCE WATCHDOG: Triggering coaching for non-final transcript`)
  ...
  useEffect(() => {
    ...
    if (salespersonStream.lastFinal) {
      addLog(`≡ƒÄñ SALES transcript: "${salespersonStream.lastFinal.text.substring(0, 30)}..."`)
  ...
  useEffect(() => {
    if (prospectStream.lastFinal) {
      ...
      addLog(`≡ƒÄñ PROSPECT transcript: "${prospectStream.lastFinal.text.substring(0, 30)}..."`)
  ...
  useEffect(() => {
    ...
    addLog(`ΓÜí Instant coaching from partial prospect speech`)
  ...
  useEffect(() => {
    if (prospectStream.isConnected) {
      ...
      if (trace.A) {
        addLog(`ΓÜá∩╕Å Prospect stream DISCONNECTED`)
        updateTrace({ A: false })
      }
    }
  ...
  useEffect(() => {
    if (salespersonStream.error) {
      addLog(`Γ¥î STT Error: ${salespersonStream.error}`)
    }
  ...
  const setupProspectStream = async () => {
    try {
      ...
      if (!stream) {
        addLog("Γ¥î Failed to get display media stream")
        return null
      }

      const audioTrack = stream.getAudioTracks()[0]
      if (!audioTrack) {
        addLog("Γ¥î No audio track! User missed checkbox?")
        alert("ΓÜá∩╕Å IMPORTANT: Check 'Share tab audio' in the popup to capture prospect audio!")
        stream.getTracks().forEach((t: MediaStreamTrack) => t.stop())
        return null
      }

      addLog("Γ£à Audio track captured")
      ...
      audioTrack.onended = () => {
        addLog("ΓÜá∩╕Å Audio track ended (but stream continues)")
      ...
      addLog(`Γ¥î Permission denied: ${err}`)
      return null
    }
  }
  ...
  useEffect(() => {
    if (status !== "listening" || !isDiarized || manualSpeaker !== 'prospect') {
      // Not in prospect mode ΓÇö clear silence tracking
    ...
    const interval = setInterval(() => {
      if (isSpeakingRef.current) {
        // Someone is talking ΓÇö reset silence timer
        silenceWatchdogRef.current = null
        silenceResetDoneRef.current = false
        return
      }

      // Silent ΓÇö start or continue tracking
      ...
      if (silenceDuration >= 10000 && !silenceResetDoneRef.current) {
        // 10 seconds of silence ΓÇö trigger ONE hardware reset
        console.warn(`[IN-ROOM WATCHDOG] ≡ƒöä ${Math.round(silenceDuration / 1000)}s silence detected ΓÇö Refreshing audio...`)
        addLog("ΓÜí SYSTEM: HARDWARE RESET (10s Silence Watchdog)...")
  ...
  useEffect(() => {
    if (status === "listening" && isDiarized && manualSpeaker === 'prospect') {
      addLog("ΓÜí SYSTEM: HARDWARE RESET (Speaker Switch to Prospect)...")
      console.warn("[IN-ROOM WATCHDOG] ≡ƒöä Refreshing audio pulse to prevent timeout...")
      salespersonStream.startAutomatic()
    }
  ...
  const handleStartCoaching = useCallback(async (mode: 'dual' | 'diarized' = 'dual') => {
    ...
    if (diarize) {
      addLog("≡ƒÜÇ INITIALIZING IN-ROOM CAPTURE (Diarization V1.2)...")
      ...
      try {
        ...
        addLog("Γ£à WEBSOCKET CONNECTED - Port 3002")
        addLog("≡ƒÄÖ∩╕Å CAPTURE ACTIVE: Use the buttons to switch speakers.")
      } catch (e: any) {
        addLog(`Γ¥î Diarization failed: ${e.message || e}`)
        setStatus("ready")
      }
    } else {
      ...
      if (!prospectResult) {
        addLog("Γ¥î CRITICAL: Prospect stream failed - no prospect audio captured!")
        setCards([{
          id: 'error-' + Date.now(),
          suggestion: "ΓÜá∩╕Å Prospect audio not captured",
