...
async function startServer() {
  ...
  if (!PORT) {
    console.error(`[WS Server] Γ¥î ERROR: All ports ${PORT_RANGE_START}-${PORT_RANGE_END} are in use!`);
          ...
          if (data.results && data.results.length > 0) {
            ...
            if (alternative?.transcript) {
              ...
              if (ws.readyState === WebSocket.OPEN) {
                ...
                console.log(`[WS Server] ${event.type === 'final' ? 'Γ£à' : '≡ƒô¥'} ${session.speaker}: "${event.text.substring(0, 60)}..." (data events: ${dataEventCount}, finals: ${finalCount})`);
              } else {
      ...
      try {
        ...
        if (buffer) {
          ...
          if (writeCount % 500 === 0) {
            ...
            console.log(`[WS Server ${sessionId}] ≡ƒôê Data received: ${mb}MB (${writeCount} chunks)`);
          }
