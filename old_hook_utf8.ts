...
export function useSTTStream(
  ...
  // Connect to WebSocket server with auto port discovery (3002-3010)
  const connectWebSocket = useCallback(async (speaker: 'salesperson' | 'prospect', sessionId: string, diarize = false): Promise<WebSocket> => {
    const tryPort = (port: number): Promise<WebSocket> => {
      return new Promise((resolve, reject) => {
        ...
        const ws = new WebSocket(`ws://localhost:${port}?session=${sessionId}&speaker=${speaker}${diarize ? '&diarize=true' : ''}`)
        ...
        ws.onopen = () => {
          ...
          console.log(`[TRACE-A] ${speaker} - WebSocket CONNECTED on port ${port}`)
          resolve(ws)
        }
  ...
  const startStream = useCallback(async (speaker: 'salesperson' | 'prospect', audioStream?: MediaStream, diarize = false) => {
    try {
        ...
        if (isStreaming && reconnectAttemptsRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          setTimeout(async () => {
            try {
              ...
              console.error(`[WS STT ${speaker}] Γ¥î Reconnect failed:`, err)
      ...
      scriptProcessor.onaudioprocess = (audioEvent) => {
        ...
        if (now % 5000 < 100) {
          console.log(`[WS STT ${speaker}] ≡ƒÄ╡ Audio Pulse: capturing... (RMS: ${avgRMS.toFixed(4)})`)
        }
        ...
        while (pcmAccumulator.length >= sendBufferSize) {
          ...
          if (wsToSend && wsToSend.readyState === WebSocket.OPEN) {
            ...
            if (Math.floor(bytesSentRef.current / (1024 * 1024)) > Math.floor((bytesSentRef.current - pcmData.byteLength) / (1024 * 1024))) {
              ...
              console.log(`[WS STT ${speaker}] ≡ƒôñ Total audio sent: ${totalMB}MB`)
            }
          }
        }
      }
      ...
      watchdogIntervalRef.current = setInterval(() => {
        ...
        if (isStreamingRef.current && timeSinceLastAudio > 5000) {
          console.warn(`[WATCHDOG] ${speaker} - ≡ƒöÑ NO AUDIO CAPTURE FOR ${timeSinceLastAudio}ms. FORCE RESTARTING...`)
          startAutomatic()
        }
      }, 2000)

      console.log(`[WS STT ${speaker}] Γ£à Audio processing started (stream active: ${audioStreamToUse.active})`)
    } catch (err) {
      console.error(`[WS STT ${speaker}] Γ¥î Error starting stream:`, err)
  ...
  const startAutomatic = useCallback(async () => {
    ...
    console.log(`[TRACE-AUTO] ${speaker} - ≡ƒöä Starting NEW stream automatically (replacing ${oldSessionId})...`)
    ...
    try {
      ...
      console.log(`[TRACE-AUTO] ${speaker} - Γ£à AUTO-START COMPLETE (Context Recreated)`)
    } catch (err) {
      console.error(`[TRACE-AUTO] ${speaker} - Γ¥î AUTO-START FAILED:`, err)
